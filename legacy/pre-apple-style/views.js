(function (app) {
  ...
  function pageHeader(title, subtitle, backRoute) {
    return `<header class="page-header">${backRoute ? `<a class="back-button" href="#/${backRoute}" data-route="${backRoute}" aria-label="返回">${icon("arrow")}</a>` : ""}<div><h1>${escapeHtml(title)}</h1>${subtitle ? `<p>${escapeHtml(subtitle)}</p>` : ""}</div></header>`;
  }
  function onboarding(selectedLanguage) {
    return `<section class="onboarding page-narrow"><div class="onboarding-mark" aria-hidden="true">旅</div><p class="eyebrow">旅行单词</p><h1>先选择这次旅行要学习的语言</h1><p class="lead">之后可以随时在“我的”中切换。无需注册，学习设置只保存在这台设备。</p><div class="language-options" role="radiogroup" aria-label="学习语言">${app.catalog.LANGUAGES.map((language) => `<button class="language-option${selectedLanguage === language.id ? " selected" : ""}" type="button" role="radio" aria-checked="${selectedLanguage === language.id}" data-onboarding-language="${language.id}"><span><strong>${language.label}</strong><small>${language.nativeLabel}</small></span><span class="radio-indicator" aria-hidden="true"></span></button>`).join("")}</div><button class="button primary full" type="button" data-complete-onboarding>开始使用</button></section>`;
  }
  ...
  function learn(language, pwaState, installDismissed) {
    ...
    return `<div class="page page-with-nav"><header class="learn-header"><div><p class="eyebrow">旅行单词</p><h1>学习</h1></div><a class="language-chip" href="#/me" data-route="me">${language.label}</a></header><p class="lead compact">选择眼前需要的场景，直接学习能在旅途中用上的表达。</p>${installBanner(pwaState, installDismissed)}<section class="section-block"><div class="section-heading"><h2>基础</h2><span>可选</span></div><a class="feature-row" href="#/learn/foundation" data-route="learn/foundation"><span class="row-icon">${icon("foundation")}</span><span class="row-copy"><strong>${foundation.title}</strong><small>${foundation.description}</small></span>${icon("chevron", "chevron")}</a></section><section class="section-block"><div class="section-heading"><h2>旅行场景</h2><span>自由浏览</span></div><div class="scenario-list">${app.catalog.SCENARIOS.map((scenario) => { const counts = app.catalog.scenarioCounts(scenario.id); const status = scenario.status === "ready" ? `${counts.total} 条内容` : "待人工整理"; return `<a class="scenario-row" href="#/learn/${scenario.id}" data-route="learn/${scenario.id}"><span class="row-icon">${icon(scenario.icon)}</span><span class="row-copy"><strong>${scenario.title}</strong><small>${scenario.description}</small></span><span class="row-meta">${status}</span>${icon("chevron", "chevron")}</a>`; }).join("")}</div></section></div>`;
  }
  function foundation() {
    return `<div class="page page-with-nav">${pageHeader("旅行基础", "这是可选入口，不会限制任何旅行场景。", "learn")}<section class="notice-panel"><h2>内容正在按用途整理</h2><p>现有词库中包含数字、时间和礼貌表达，但这些内容也会用于交通、景点和日常沟通。为了避免重复或错误归类，目前暂不直接合并。</p><p class="muted">原始内容仍完整保存在词库中，后续会逐条完成场景标注。</p></section></div>`;
  }
  ...
  function scenarioPage(scenario, language, selectedTab) {
    if (scenario.status !== "ready") return `<div class="page page-with-nav">${pageHeader(scenario.title, scenario.description, "learn")}<section class="notice-panel"><h2>这个场景还需要人工整理</h2><p>候选内容分散在多个旧分类中，或与其他场景混在一起。为了不重复、不猜测词条归属，目前先保留真实入口，不展示未经确认的内容。</p><p class="muted">候选来源：${scenario.candidateSourceCategories.map(escapeHtml).join("、")}</p></section></div>`;
  ...
  function me(language, summary, pwaState) {
    ...
    return `<div class="page page-with-nav">${pageHeader("我的", "设置和学习记录只保存在当前浏览器。")}
      ...
      <section class="settings-section"><h2>本地记录</h2><dl class="legacy-summary"><div><dt>${summary.learned}</dt><dd>已有学习记录</dd></div><div><dt>${summary.wrong}</dt><dd>旧版错题</dd></div></dl><button class="button secondary" type="button" data-reset-learning>清除学习记录</button></section>
      <section class="settings-section"><h2>安装应用</h2><p>${installCopy}</p>${pwaState.type === "native" ? '<button class="button secondary" type="button" data-install>安装旅行单词</button>' : ""}</section></div>`;
  }
