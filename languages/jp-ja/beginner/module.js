(function () {
  ...
  function normalizeProgress(raw) {
    ...
    const lessons = (window.BEGINNER_DATA?.stages || []).flatMap((stage) => stage.lessons || []);
  ...
  function getHomeSummary(progress) { return progress.placementPassed ? "已通过认读测试" : progress.challengeDone ? "认读挑战已完成" : "从假名开始，尝试读出日语"; }
  function validateData() {
    return !window.BEGINNER_DATA || window.BEGINNER_DATA.stages?.length !== 5 || window.BEGINNER_DATA.stages.flatMap((stage) => stage.lessons || []).length !== 18 || window.BEGINNER_DATA.placement?.length !== 12 || window.BEGINNER_DATA.challenge?.length !== 6
      ? "日语认读课程数据不完整。 " : "";
  }
  let renderer = null;
  function createRenderer(ctx) {
    ...
    function beginnerLessons() { return window.BEGINNER_DATA.stages.flatMap((stage) => stage.lessons); }
    ...
    function renderBeginnerOverview() {
      ...
      const sections = window.BEGINNER_DATA.stages.map((stage, index) => `<section class="beginner-stage"><div class="section-heading"><div><span class="eyebrow">阶段 ${index + 1}</span><h2>${escapeHtml(stage.title)}</h2><p>${escapeHtml(stage.subtitle)}</p></div><span>${stage.lessons.length} 节短课</span></div>
        <div class="beginner-lesson-list">${stage.lessons.map((lesson) => `<button class="beginner-lesson-row" type="button" data-lesson="${lesson.id}"><span class="lesson-mark">${beginner.completedLessons.includes(lesson.id) ? "✓" : "○"}</span><span><strong>${escapeHtml(lesson.title)}</strong><small>${escapeHtml(lesson.note || `${lesson.pairs.length} 个声音 · 平假名与片假名同步认识`)}</small></span><span aria-hidden="true">›</span></button>`).join("")}</div></section>`).join("");
      view.innerHTML = `<div class="page-enter beginner-page">${beginnerHeader("零基础认读", "从假名与声音开始，练到能自己尝试读日语单词和短句。")} 
        <section class="beginner-intro-card"><span class="eyebrow">你的进度</span><h2>${beginner.placementPassed ? "已通过认读测试" : beginner.challengeDone ? "认读挑战已完成" : `已完成 ${count} / ${lessons.length} 节短课`}</h2><p>课程只练文字到声音，不需要写字或使用麦克风。</p><div class="beginner-intro-actions"><button class="primary-btn" type="button" data-start-beginner>${next ? "继续短课" : "查看认读挑战"}</button><button class="secondary-btn compact" type="button" data-test-beginner>认读测试</button></div></section>
        ${sections}<section class="beginner-stage"><div class="section-heading"><div><h2>结课认读挑战</h2></div></div><p class="beginner-stage-copy">先自己读，再听标准音频核对。</p><button class="secondary-btn" type="button" data-challenge ${next ? "disabled" : ""}>${next ? "完成短课后开始" : "进入挑战"}</button></section></div>`;
      bindBeginnerBack();
    ...
    function renderPlacementIntro() {
      stopSpeech();
      view.innerHTML = `<div class="page-enter beginner-page">${beginnerHeader("零基础认读测试", "已有基础？用 12 道题检查假名、拼读规则和连续认读。")} 
        <section class="beginner-intro-card"><h2>先听、再看、再判断</h2><p>至少答对 10 题，且每个类别至少答对一题，就能跳过短课。</p><button class="primary-btn" type="button" data-start-placement>开始测试</button></section></div>`;
      bindBeginnerBack();
    ...
    function renderPlacementResult() {
      ...
      const categories = Array.from(new Set(window.BEGINNER_DATA.placement.map((item) => item.category)));
      ...
      view.innerHTML = `<div class="page-enter beginner-page">${beginnerHeader("测试结果", "这次测试仅用来选择学习路线。")}
        <section class="beginner-intro-card"><span class="beginner-score">${score} / 12</span><h2>${passed ? "你已经具备基础认读能力" : "建议从短课练起"}</h2><p>${passed ? "可以跳过认读短课。" : "假名、拼读规则或连续认读还有不熟悉的地方。"}</p><div class="beginner-intro-actions"><button class="primary-btn" type="button" data-result-home>返回首页</button><button class="secondary-btn compact" type="button" data-result-lessons>${passed ? "仍想练习" : "开始短课"}</button></div></section></div>`;
      bindBeginnerBack();
    ...
    function lessonQuestions(lesson) {
      if (lesson.question) {
        ...
        if (lesson.example) {
          const other = beginnerLessons().filter((item) => item.example && item.example.text !== lesson.example.text).slice(0, 2);
    ...
    function renderBeginnerLesson(lessonId) {
      ...
      const isRule = Boolean(lesson.question) && !isReading;
      const count = beginnerLessons().findIndex((item) => item.id === lesson.id) + 1;
      const cards = isReading
        ? lesson.pairs.map(([text, reading], index) => `<div class="beginner-kana-card"><strong>${escapeHtml(text)}</strong><small>${escapeHtml(reading)}</small>${beginnerSpeaker(text, lesson.example?.text === text ? lesson.example.audioId : `beginner/reading-${lesson.id}-${index}`, `播放${text}的读音`)}</div>`).join("")
        : isRule
      ...
      view.innerHTML = `<div class="page-enter beginner-page">${beginnerHeader(lesson.title, `短课 ${count} / ${beginnerLessons().length} · ${isReading ? "连续认读" : isRule ? "拼读规则" : "基础假名"}`)}
        <section class="beginner-intro-card"><h2>${isReading ? "先分清字形，再连续读出" : isRule ? "这个规则怎样改变读音？" : "一个声音，两种字形"}</h2><p>${escapeHtml(isReading || isRule ? lesson.note : "先看平假名，再看对应的片假名。点击播放，听它们共同的读音。")}</p><div class="beginner-kana-grid">${cards}</div>${example}<button class="primary-btn" type="button" data-lesson-practice>开始练习</button></section></div>`;
      bindBeginnerBack(); bindBeginnerSpeakers();
    ...
    function renderChallengeIntro() {
      ...
      view.innerHTML = `<div class="page-enter beginner-page">${beginnerHeader("结课认读挑战", "依次读出四个单词和两个短句，再听标准音频核对。")} 
    ...
    function renderChallengeWord(ready = false, played = false) {
      ...
      view.innerHTML = `<div class="page-enter beginner-page">${beginnerHeader("结课认读挑战", `第 ${session.index + 1} / ${window.BEGINNER_DATA.challenge.length} 项`)}
    ...
    function renderChallengeResult() {
      ...
      saveBeginnerState(beginner);
      view.innerHTML = `<div class="page-enter beginner-page">${beginnerHeader("挑战完成", "你已经尝试不靠罗马音读出日语单词和短句。")} 
        <section class="beginner-intro-card"><span class="beginner-score">✓</span><h2>你已完成这套零基础认读课程</h2><p>${session.retries.length ? `有 ${session.retries.length} 项想再练一次，随时可以重做挑战。` : "六项内容都已核对，可以继续学习场景词汇和短句。"}</p><div class="beginner-intro-actions"><button class="primary-btn" type="button" data-finish-home>返回首页</button><button class="secondary-btn compact" type="button" data-finish-overview>查看短课</button></div></section></div>`;
      bindBeginnerBack();
